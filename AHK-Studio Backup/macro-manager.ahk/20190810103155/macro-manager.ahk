...
Loop, Parse, SectionEntries, `n
{
	...
	Cases := CaseKeys(SplitKey)
	MakeHotstrings(Cases)
}

CaseKeys(SplitKey)
{
	Cases := {}
	Cases["lower"] := Format("{1:l}",      Splitkey[1])
	Cases["upper"] := Format("{1:U}{2:l}", SubStr(SplitKey[1], 1, 1), SubStr(SplitKey[1], 2))
	Cases["titled"]  := Format("{1:U}",      SplitKey[1])
	return Cases
}
...
{
	C := Cases
	Hotstring(":CX:" C["lower"] " ", "PasteText")
	Hotstring(":CX:" C["upper"] " ", "PasteUpper")	
	Hotstring(":CX:" C["titled"] " ", "PasteTitled")
	return
}
